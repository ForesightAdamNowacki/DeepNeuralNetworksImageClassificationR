...
# 3. Create 'Categorical' subfolder in 'VGG19' main folder
...
last_model <- base::list.files(path = models_store_dir, pattern = ".hdf5")
model <- keras::load_model_hdf5(filepath = paste(models_store_dir, last_model, sep = "/"), compile = FALSE)

base::cat("Trainable layers before unfreezing:", base::length(model$trainable_weights), "\n")
model %>% keras::unfreeze_weights()
base::cat("Trainable layers after unfreezing:", base::length(model$trainable_weights), "\n")
# ------------------------------------------------------------------------------
# Visualize model:
