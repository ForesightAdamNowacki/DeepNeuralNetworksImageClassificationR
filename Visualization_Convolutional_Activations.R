# ------------------------------------------------------------------------------
# ACTIVATION LAYERS VISUALISATION
# ------------------------------------------------------------------------------
# Data:
...
# ------------------------------------------------------------------------------
# Model name:
model_name <- "Visualisation_Convolutional_Activations"
...
# ------------------------------------------------------------------------------
# Function Implementation for Visualisation Convolutional 2D Layers Activations:
Convolutional_2D_Layers_Activations <- function(model,
                                                image_path,
                                                layer_id,
                                                save_plot = FALSE){
  ...
  layer_outputs <- lapply(model$layers[grepl('convolutional', sapply(model$layers, paste))], function(layer) layer$output)
  ...
  if(save_plot == TRUE){
    file_name <- paste(paste(stringr::str_replace_all(Sys.time(), ":", "-"), "Convolutional_2D_Layer_Visualisation", layer_id, sep = "_"), "png", sep = ".")
