# ------------------------------------------------------------------------------
# NASNETMOBILE CATEGORICAL MODEL IMPLEMENTATION
# ------------------------------------------------------------------------------
# Data
...
# 3. Create 'Categorical' subfolder in 'NASNetMobile' main folder
base::dir.create(path = base::paste(base::getwd(), "NASNetMobile", "Categorical", sep = "/"))
...
models_store_dir <- base::paste(base::getwd(), "NASNetMobile/Categorical", sep = "/")
...
# 3. cd D:\GitHub\DeepNeuralNetworksRepoR\NASNetMobile\Categorical
...
# model <- keras::load_model_hdf5(filepath = "D:/GitHub/DeepNeuralNetworksRepoR_Models_Store/Categorical_NASNetMobile_Model.hdf5", compile = FALSE)
model %>% keras::compile(loss = loss,
                         optimizer = optimizer, 
                         metrics = metrics)
...
readr::write_csv2(tibble::as_tibble(train_probabilities) %>%
                    dplyr::mutate(filepath = train_generator$filepaths), base::paste(datetime, "NASNetMobile_train_categorical_probabilities.csv"))
readr::write_csv2(tibble::as_tibble(validation_probabilities) %>%
                    dplyr::mutate(filepath = validation_generator$filepaths), base::paste(datetime, "NASNetMobile_validation_categorical_probabilities.csv"))
readr::write_csv2(tibble::as_tibble(test_probabilities) %>%
                    dplyr::mutate(filepath = test_generator$filepaths), base::paste(datetime, "NASNetMobile_test_categorical_probabilities.csv"))
