# ------------------------------------------------------------------------------
# INCEPTION RESNET V2 CATEGORICAL MODEL IMPLEMENTATION
# ------------------------------------------------------------------------------
# Data
...
# 3. Create 'Categorical' subfolder in 'Inception_ResNet_V2' main folder
base::dir.create(path = base::paste(base::getwd(), "Inception_ResNet_V2", "Categorical", sep = "/"))
...
models_store_dir <- base::paste(base::getwd(), "Inception_ResNet_V2/Categorical", sep = "/")
...
# 3. cd D:\GitHub\DeepNeuralNetworksRepoR\Inception_ResNet_V2\Categorical
...
# model <- keras::load_model_hdf5(filepath = "D:/GitHub/DeepNeuralNetworksRepoR_Models_Store/Categorical_Inception_ResNet_V2_Model.hdf5", compile = FALSE)
model %>% keras::compile(loss = loss,
                         optimizer = optimizer, 
                         metrics = metrics)
...
readr::write_csv2(tibble::as_tibble(train_probabilities) %>%
                    dplyr::mutate(filepath = train_generator$filepaths), base::paste(datetime, "Inception_ResNet_V2_train_categorical_probabilities.csv"))
readr::write_csv2(tibble::as_tibble(validation_probabilities) %>%
                    dplyr::mutate(filepath = validation_generator$filepaths), base::paste(datetime, "Inception_ResNet_V2_validation_categorical_probabilities.csv"))
readr::write_csv2(tibble::as_tibble(test_probabilities) %>%
                    dplyr::mutate(filepath = test_generator$filepaths), base::paste(datetime, "Inception_ResNet_V2_test_categorical_probabilities.csv"))
...
Categorical_test_results <- Categorical_Model_Evaluation(actual = test_actual,
                                                         ...
                                                         labels = labels,
                                                         type_info = "Test Inception ResNet V2",
                                                         save = TRUE,
                                                         open = FALSE)
