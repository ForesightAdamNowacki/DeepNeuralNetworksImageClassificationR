...
# ------------------------------------------------------------------------------
# Model verification:
labels <- list.files(cv_data_dir); labels
save_option <- FALSE
...
cv_results %>%
  ...
  dplyr::summarise(actual_class = mean(actual_class),
                   prediction = mean(prediction)) %>%
  dplyr::mutate(predicted_class = as.integer(stringr::str_sub(predicted_class, 2, -1))) %>%
  dplyr::arrange(filepath, predicted_class) %>%
  tidyr::pivot_wider(id_cols = c("filepath", "actual_class"),
