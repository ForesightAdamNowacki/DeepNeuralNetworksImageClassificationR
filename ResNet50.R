...
callback_model_checkpoint_path <- "C:\\Users\\admin\\Desktop\\GitHub\\Models_Store\\ResNet50-Models\\keras_model.weights.{epoch:02d}-{val_acc:.2f}.hdf5"
callback_tensorboard_path <- "C:\\Users\\admin\\Desktop\\GitHub\\Models_Store\\ResNet50-Models"
callback_csv_logger_path <- "C:\\Users\\admin\\Desktop\\GitHub\\Models_Store\\ResNet50-Models\\Optimization_logger.csv"
...
resnet50_model <- keras::application_resnet50(include_top = FALSE, weights = NULL, input_shape = base::c(224, 224, 3))

input_tensor <- keras::layer_input(shape = base::c(224, 224, 3))
...
resnet50_model <- keras::keras_model(inputs = input_tensor, outputs = output_tensor)
resnet50_model

# ------------------------------------------------------------------------------
# Upload pre-trained model for training:
models_store <- "C:\\Users\\admin\\Desktop\\GitHub\\Models_Store\\ResNet50-Models"
last_model <- base::list.files(path = models_store, pattern = ".hdf5")[base::length(base::list.files(path = models_store, pattern = ".hdf5"))]
resnet50_model <- keras::load_model_hdf5(filepath = paste(models_store, last_model, sep = "\\"), compile = TRUE)

# ------------------------------------------------------------------------------
# Model compilation:
resnet50_model %>% keras::compile(
...
# ------------------------------------------------------------------------------
# Model optimization:
history <- resnet50_model %>% keras::fit_generator(
