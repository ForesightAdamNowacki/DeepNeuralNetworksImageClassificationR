...
readr::write_csv2(tibble::as_tibble(train_probabilities), base::paste(datetime, "VGG16_train_binary_probabilities.csv"))
readr::write_csv2(tibble::as_tibble(validation_probabilities), base::paste(datetime, "VGG16_validation_binary_probabilities.csv"))
readr::write_csv2(tibble::as_tibble(test_probabilities), base::paste(datetime, "VGG16_test_binary_probabilities.csv"))
...
train_verification_1$Assessment_of_Classifier_Effectiveness %>%
  ...
  dplyr::mutate(Score_validation = validation_verification_1$Assessment_of_Classifier_Effectiveness$Score,
                Score_test = test_verification_1$Assessment_of_Classifier_Effectiveness$Score,
                Cutoff = default_cutoff) %>%
  readr::write_csv2(path = base::paste(models_store_dir, "Summary_Default_Cutoff_VGG16.csv", sep = "/"))
...
train_verification_2$Assessment_of_Classifier_Effectiveness %>%
  ...
  dplyr::mutate(Score_validation = validation_verification_2$Assessment_of_Classifier_Effectiveness$Score,
                Score_test = test_verification_2$Assessment_of_Classifier_Effectiveness$Score,
                Cutoff = selected_cutoff) %>%
  readr::write_csv2(path = base::paste(models_store_dir, "Summary_Optimized_Cutoff_VGG16.csv", sep = "/"))
