...
# 2. Create 'Inception_ResNet_V2' folder in cwd
base::dir.create(path = base::paste(base::getwd(), "Inception_ResNet_V2", sep = "/"))
# 3. Create 'Binary' subfolder in 'Inception_ResNet_V2' main folder
base::dir.create(path = base::paste(base::getwd(), "Inception_ResNet_V2", "Binary", sep = "/"))
...
# model <- keras::load_model_hdf5(filepath = "D:/GitHub/DeepNeuralNetworksRepoR_Models_Store/Binary_Inception_ResNet_V2_Model.hdf5", compile = FALSE)
model %>% keras::compile(loss = loss,
                         optimizer = optimizer, 
                         metrics = metrics)
...
readr::write_csv2(tibble::as_tibble(train_probabilities) %>%
                    dplyr::mutate(filepath = train_generator$filepaths), base::paste(datetime, "Inception_ResNet_V2_train_binary_probabilities.csv"))
readr::write_csv2(tibble::as_tibble(validation_probabilities) %>%
                    dplyr::mutate(filepath = validation_generator$filepaths), base::paste(datetime, "Inception_ResNet_V2_validation_binary_probabilities.csv"))
readr::write_csv2(tibble::as_tibble(test_probabilities) %>%
                    dplyr::mutate(filepath = test_generator$filepaths), base::paste(datetime, "Inception_ResNet_V2_test_binary_probabilities.csv"))
...
train_verification_1$Assessment_of_Classifier_Effectiveness %>%
  ...
  dplyr::mutate(Score_validation = validation_verification_1$Assessment_of_Classifier_Effectiveness$Score,
                Score_test = test_verification_1$Assessment_of_Classifier_Effectiveness$Score,
                Cutoff = default_cutoff) %>%
  readr::write_csv2(path = base::paste(models_store_dir, base::paste(datetime, "Summary_Default_Cutoff_Inception_ResNet_V2.csv"), sep = "/"))
...
train_verification_2$Assessment_of_Classifier_Effectiveness %>%
  ...
  dplyr::mutate(Score_validation = validation_verification_2$Assessment_of_Classifier_Effectiveness$Score,
                Score_test = test_verification_2$Assessment_of_Classifier_Effectiveness$Score,
                Cutoff = selected_cutoff) %>%
  readr::write_csv2(path = base::paste(models_store_dir, base::paste(datetime, "Summary_Optimized_Cutoff_Inception_ResNet_V2.csv"), sep = "/"))
...
final_score_1_summary %>%
  ...
  dplyr::mutate(Train_diff = Train_optimized - Train_default,
                ...
                Test_diff = Test_optimized - Test_default,
                Cutoff = selected_cutoff) %>%
  readr::write_csv2(path = base::paste(models_store_dir, base::paste(datetime, "Summary_Comparison_Default_Optimized_Cutoff_Inception_ResNet_V2.csv"), sep = "/"))
