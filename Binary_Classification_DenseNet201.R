# ------------------------------------------------------------------------------
# DENSENET201 BINARY MODEL IMPLEMENTATION
# ------------------------------------------------------------------------------
# Data:
...
# 3. Create 'Binary' subfolder in 'DenseNet201' main folder
base::dir.create(path = base::paste(base::getwd(), "DenseNet201", "Binary", sep = "/"))
...
models_store_dir <- "D:/GitHub/DeepNeuralNetworksRepoR/DenseNet201/Binary"
...
# 3. cd D:/GitHub/DeepNeuralNetworksRepoR/DenseNet201/Binary
...
# model <- keras::load_model_hdf5(filepath = "D:/GitHub/DeepNeuralNetworksRepoR_Models_Store/Binary_DenseNet201_Model.hdf5", compile = FALSE)
model %>% keras::compile(loss = loss,
                         optimizer = optimizer, 
                         metrics = metrics)
...
readr::write_csv2(tibble::as_tibble(train_probabilities) %>%
                    dplyr::mutate(filepath = train_generator$filepaths), base::paste(datetime, "DenseNet201_train_binary_probabilities.csv"))
readr::write_csv2(tibble::as_tibble(validation_probabilities) %>%
                    dplyr::mutate(filepath = validation_generator$filepaths), base::paste(datetime, "DenseNet201_validation_binary_probabilities.csv"))
readr::write_csv2(tibble::as_tibble(test_probabilities) %>%
                    dplyr::mutate(filepath = test_generator$filepaths), base::paste(datetime, "DenseNet201_test_binary_probabilities.csv"))
...
train_verification_1 <- Binary_Classifier_Verification(actual = train_actual,
                                                       ...
                                                       type_info = "Train DenseNet201 default cutoff",
                                                       save = TRUE,
                                                       open = FALSE)
...
validation_verification_1 <- Binary_Classifier_Verification(actual = validation_actual,
                                                            ...
                                                            cutoff = default_cutoff,
                                                            type_info = "Validation DenseNet201 default cutoff",
                                                            save = TRUE,
                                                            open = FALSE)
...
test_verification_1 <- Binary_Classifier_Verification(actual = test_actual,
                                                      ...
                                                      cutoff = default_cutoff,
                                                      type_info = "Test DenseNet201 default cutoff",
                                                      save = TRUE,
                                                      open = FALSE)
...
train_verification_1$Assessment_of_Classifier_Effectiveness %>%
  ...
  dplyr::mutate(Score_validation = validation_verification_1$Assessment_of_Classifier_Effectiveness$Score,
                Score_test = test_verification_1$Assessment_of_Classifier_Effectiveness$Score,
                Cutoff = default_cutoff) %>%
  readr::write_csv2(path = base::paste(models_store_dir, base::paste(datetime, "Summary_Default_Cutoff_DenseNet201.csv"), sep = "/"))
...
train_verification_2 <- Binary_Classifier_Verification(actual = train_actual,
                                                       ...
                                                       type_info = "Train DenseNet201 optimized cutoff",
                                                       save = TRUE,
                                                       open = FALSE)
...
validation_verification_2 <- Binary_Classifier_Verification(actual = validation_actual,
                                                            predicted = validation_predicted,
                                                            cutoff = selected_cutoff,
                                                            type_info = "Validation DenseNet201 optimized cutoff",
                                                            save = TRUE,
                                                            open = FALSE)
...
test_verification_2 <- Binary_Classifier_Verification(actual = test_actual,
                                                      predicted = test_predicted,
                                                      cutoff = selected_cutoff,
                                                      type_info = "Test DenseNet201 optimized cutoff",
                                                      save = TRUE,
                                                      open = FALSE)
...
train_verification_2$Assessment_of_Classifier_Effectiveness %>%
  ...
  dplyr::mutate(Score_validation = validation_verification_2$Assessment_of_Classifier_Effectiveness$Score,
                Score_test = test_verification_2$Assessment_of_Classifier_Effectiveness$Score,
                Cutoff = selected_cutoff) %>%
  readr::write_csv2(path = base::paste(models_store_dir, base::paste(datetime, "Summary_Optimized_Cutoff_DenseNet201.csv"), sep = "/"))
...
final_score_1_summary %>%
  ...
  dplyr::mutate(Train_diff = Train_optimized - Train_default,
                ...
                Test_diff = Test_optimized - Test_default,
                Cutoff = selected_cutoff) %>%
  readr::write_csv2(path = base::paste(models_store_dir, base::paste(datetime, "Summary_Comparison_Default_Optimized_Cutoff_DenseNet201.csv"), sep = "/"))
