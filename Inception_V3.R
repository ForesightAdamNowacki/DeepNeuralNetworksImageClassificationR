# ------------------------------------------------------------------------------
# INCEPTION_V3 MODEL IMPLEMENTATION
# ------------------------------------------------------------------------------
# Data
...
callback_model_checkpoint_path <- "C:\\Users\\admin\\Desktop\\GitHub\\Models_Store\\Inception-V3-Models\\keras_model.weights.{epoch:02d}-{val_acc:.2f}.hdf5"
callback_tensorboard_path <- "C:\\Users\\admin\\Desktop\\GitHub\\Models_Store\\Inception-V3-Models"
callback_csv_logger_path <- "C:\\Users\\admin\\Desktop\\GitHub\\Models_Store\\Inception-V3-Models\\Optimization_logger.csv"
...
inception_v3_model <- keras::application_inception_v3(include_top = FALSE, weights = NULL, input_shape = base::c(299, 299, 3))

input_tensor <- keras::layer_input(shape = base::c(299, 299, 3))
...
inception_v3_model <- keras::keras_model(inputs = input_tensor, outputs = output_tensor)
inception_v3_model

# ------------------------------------------------------------------------------
# Upload pre-trained model for training:
models_store <- "C:\\Users\\admin\\Desktop\\GitHub\\Models_Store\\Inception-V3-Models"
last_model <- base::list.files(path = models_store, pattern = ".hdf5")[base::length(base::list.files(path = models_store, pattern = ".hdf5"))]
inception_v3_model <- keras::load_model_hdf5(filepath = paste(models_store, last_model, sep = "\\"), compile = TRUE)

# ------------------------------------------------------------------------------
# Model compilation:
inception_v3_model %>% keras::compile(
...
# ------------------------------------------------------------------------------
# Model optimization:
history <- inception_v3_model %>% keras::fit_generator(
