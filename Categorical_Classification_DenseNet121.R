# ------------------------------------------------------------------------------
# DENSENET121 CATEGORICAL MODEL IMPLEMENTATION
# ------------------------------------------------------------------------------
# Data
...
# 3. Create 'Categorical' subfolder in 'DenseNet121' main folder
base::dir.create(path = base::paste(base::getwd(), "DenseNet121", "Categorical", sep = "/"))
...
models_store_dir <- base::paste(base::getwd(), "DenseNet121/Categorical", sep = "/")
...
# 3. cd D:\GitHub\DeepNeuralNetworksRepoR\DenseNet121\Categorical
...
# model <- keras::load_model_hdf5(filepath = "D:/GitHub/DeepNeuralNetworksRepoR_Models_Store/Categorical_DenseNet121_Model.hdf5", compile = FALSE)
model %>% keras::compile(loss = loss,
                         optimizer = optimizer, 
                         metrics = metrics)
...
readr::write_csv2(tibble::as_tibble(train_probabilities) %>%
                    dplyr::mutate(filepath = train_generator$filepaths), base::paste(datetime, "DenseNet121_train_categorical_probabilities.csv"))
readr::write_csv2(tibble::as_tibble(validation_probabilities) %>%
                    dplyr::mutate(filepath = validation_generator$filepaths), base::paste(datetime, "DenseNet121_validation_categorical_probabilities.csv"))
readr::write_csv2(tibble::as_tibble(test_probabilities) %>%
                    dplyr::mutate(filepath = test_generator$filepaths), base::paste(datetime, "DenseNet121_test_categorical_probabilities.csv"))
