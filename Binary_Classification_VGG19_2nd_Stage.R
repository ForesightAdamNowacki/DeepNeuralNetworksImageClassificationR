...
callback_model_checkpoint_path <- paste(models_store_dir, "keras_model.2nd_stage_weights.{epoch:02d}-{val_acc:.4f}-{val_loss:.4f}.hdf5", sep = "/")
callback_tensorboard_path <- paste(models_store_dir, "logs_unfreeze_weights", sep = "/")
...
last_model <- list.files(path = models_store_dir, pattern = ".hdf5")
model <- keras::load_model_hdf5(filepath = paste(models_store_dir, last_model, sep = "/"), compile = FALSE)

cat("Trainable layers before unfreezing:", length(model$trainable_weights), "\n")
model %>% keras::unfreeze_weights()
cat("Trainable layers after unfreezing:", length(model$trainable_weights), "\n")
# ------------------------------------------------------------------------------
# Visualize model:
...
# 4. tensorboard --logdir=logs_unfreeze_weights --host=127.0.0.1
