# ------------------------------------------------------------------------------
# XCEPTION CATEGORICAL MODEL IMPLEMENTATION
# ------------------------------------------------------------------------------
# Data
...
# 3. Create 'Categorical' subfolder in 'Xception' main folder
base::dir.create(path = base::paste(base::getwd(), "Xception", "Categorical", sep = "/"))
...
models_store_dir <- "D:/GitHub/DeepNeuralNetworksRepoR/Xception/Categorical"
...
# 3. cd D:\GitHub\DeepNeuralNetworksRepoR\Xception\Categorical
...
optimal_model_repo_dir <- base::paste(models_repo_store_dir, "Categorical_Xception_Model.hdf5", sep = "/")
...
readr::write_csv2(tibble::as_tibble(train_probabilities) %>%
                    dplyr::mutate(filepath = train_generator$filepaths), base::paste(datetime, "Xception_train_categorical_probabilities.csv"))
readr::write_csv2(tibble::as_tibble(validation_probabilities) %>%
                    dplyr::mutate(filepath = validation_generator$filepaths), base::paste(datetime, "Xception_validation_categorical_probabilities.csv"))
readr::write_csv2(tibble::as_tibble(test_probabilities) %>%
                    dplyr::mutate(filepath = test_generator$filepaths), base::paste(datetime, "Xception_test_categorical_probabilities.csv"))
