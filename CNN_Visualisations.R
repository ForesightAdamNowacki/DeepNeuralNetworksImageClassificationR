...
Convolutional_2D_Layers_Activations <- function(model,
  ...
  if(save_plot == TRUE){
    file_name <- base::paste(base::paste(stringr::str_replace_all(base::Sys.time(), ":", "-"), "Convolutional_2D_Layer_Visualisation", layer_id, sep = "_"), "png", sep = ".")
...
Convolutional_Layers <- model$layers[base::grepl('convolutional', base::sapply(model$layers, base::paste))] %>%
