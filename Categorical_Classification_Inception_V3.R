# ------------------------------------------------------------------------------
# INCEPTION V3 CATEGORICAL MODEL IMPLEMENTATION
# ------------------------------------------------------------------------------
# Data
...
# 3. Create 'Categorical' subfolder in 'Inception_V3' main folder
base::dir.create(path = base::paste(base::getwd(), "Inception_V3", "Categorical", sep = "/"))
...
models_store_dir <- base::paste(base::getwd(), "Inception_V3/Categorical", sep = "/")
...
# 3. cd D:\GitHub\DeepNeuralNetworksRepoR\Inception_V3\Categorical
...
# model <- keras::load_model_hdf5(filepath = "D:/GitHub/DeepNeuralNetworksRepoR_Models_Store/Categorical_Inception_V3_Model.hdf5", compile = FALSE)
model %>% keras::compile(loss = loss,
                         optimizer = optimizer, 
                         metrics = metrics)
...
readr::write_csv2(tibble::as_tibble(train_probabilities) %>%
                    dplyr::mutate(filepath = train_generator$filepaths), base::paste(datetime, "Inception_V3_train_categorical_probabilities.csv"))
readr::write_csv2(tibble::as_tibble(validation_probabilities) %>%
                    dplyr::mutate(filepath = validation_generator$filepaths), base::paste(datetime, "Inception_V3_validation_categorical_probabilities.csv"))
readr::write_csv2(tibble::as_tibble(test_probabilities) %>%
                    dplyr::mutate(filepath = test_generator$filepaths), base::paste(datetime, "Inception_V3_test_categorical_probabilities.csv"))
